...
local function get_color(capacity)
	if capacity >= 60 then
		return "#90EE90"
...
local function update_battery()
	...
	if status == "Charging" then
		battery_widget:set_markup("<span foreground='yellow'> " .. capacity .. "%</span>")
	elseif status == "Full" then
		battery_widget:set_markup("<span foreground='green'>󱘖 " .. capacity .. "%</span>")
	else
		local color = get_color(capacity)
		battery_widget:set_markup("<span foreground='" .. color .. "'>" .. icon .. " " .. capacity .. "%</span>")
	end
end
